...
In this demonstration, the WINC Driver will connect to a BSS and begin a TCP client socket connection to a remote server service.<br />This demonstration can be configured in the following ways in the source code.<br />• The BSS to connect to.<br />• The TCP server address and port to connect to.<br />• The receive buffer size.<br />• The message which should be sent as the first data once connected to the server.
